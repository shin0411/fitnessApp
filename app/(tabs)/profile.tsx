...
export default function ProfileScreen() {
  ...
  async function shareInviteCode() {
    ...
    await Share.share({ message: `FitnessRPGで一緒に鵄えよう！私の招待コードは「${profile.invite_code}」です。登録時に入力すると双方に+500 XPがもらえます！` });
  }
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.inner}>
      ...
      <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        ...
        {profileDetails?.mbti && <Text style={[styles.mbti, { color: theme.primary }]}>{profileDetails.mbti}</Text>}
        {profileDetails?.height_cm && profileDetails?.weight_kg && (
          <Text style={[styles.bodyInfo, { color: theme.textSecondary }]}>{profileDetails.height_cm}cm / {profileDetails.weight_kg}kg</Text>
        )}
        {profile?.invite_code && (
          <TouchableOpacity style={[styles.inviteRow, { backgroundColor: theme.primary + '20' }]} onPress={shareInviteCode}>
      ...
      {(['physical', 'beauty', 'knowledge', 'comprehensive'] as const).map((lt) => {
        ...
        return (
          <View key={lt} style={[styles.levelRow, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <Text style={[styles.levelType, { color: theme.textSecondary }]}>{lt === 'physical' ? 'フィジカル' : lt === 'beauty' ? 'ビューティー' : lt === 'knowledge' ? 'ナレッジ' : '総合'}</Text>
      ...
      {THEMES.map((t) => (
        <TouchableOpacity key={t.id} style={[styles.themeRow, { backgroundColor: theme.surface, borderColor: themeName === t.id ? theme.primary : theme.border }]} onPress={() => setTheme(t.id)}>
          <View>
      ...
      <TouchableOpacity style={[styles.editBtn, { borderColor: theme.border }]} onPress={() => router.push('/goal-setup')}>
...
const styles = StyleSheet.create({
  ...
  editBtn: { height: 48, borderRadius: 12, borderWidth: 1, justifyContent: 'center', alignItems: 'center', marginTop: 16, marginBottom: 10 },
